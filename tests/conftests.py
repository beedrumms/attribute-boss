...
## pytest.fixture() decorator: https://docs.pytest.org/en/6.2.x/fixture.html
# @pytest.fixture(scope="session")
def MyTestingSparkSession():
    """
    ...
    """
    global spark_testing_session

    if spark_testing_session is None:

        spark_testing_session = SparkSession.builder.appName("AttributeBoss_UnitTesting").master("local").config("spark.executor.cores", "1").config("spark.executor.instances", "1").config("spark.sql.shuffle.partitions", "1").getOrCreate()
        ## config parameters essentially tell spark that you are processing on a single machine and spark should not try to distribute the computation

        print("New SparkSession has been created -->")
        print("   Master URL:", spark_testing_session.sparkContext.master)
        print("   Application Name:", spark_testing_session.sparkContext.appName)

        return spark_testing_session
        ## Note, it is recommended to yield the spark session instead of using return but it wont work
    ...
    # Teardown code after the tests
    spark_testing_session.stop()
###
...
def ExtractTestingData(TestingSparkSession=MyTestingSparkSession): 
    ...
    ("STE 300 301, 40 HOLLY ST M5R2P3 4321230194",
    "40 HOLLY ST",
    "M5R2P3",
    "4321230194")] 
